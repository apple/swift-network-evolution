...
/// Tests for the manually advanced clock behind `NetworkClock.Instant.systemNow`.
@available(Network 0.1.0, *)
final class SwiftNetworkManualClockTests: NetTestCase {
    ...
    func testSystemClockIsUsedByDefault() {
        let first = NetworkClock.Instant.systemNow
        XCTAssertNotEqual(first, .zero)
        usleep(1)
        let second = NetworkClock.Instant.systemNow
        XCTAssertGreaterThan(second, first)
    }

    func testUseManualTimeFreezesTheClock() {
        NetworkClock.Instant.useManualTime(base)
        XCTAssertEqual(NetworkClock.Instant.systemNow, base)
        ...
        usleep(1)
        XCTAssertEqual(NetworkClock.Instant.systemNow, base)
        XCTAssertEqual(NetworkClock.Instant.systemNow, NetworkClock.Instant.systemNow)
    }

    func testUseManualTimeDefaultsAbsoluteToContinuous() {
        NetworkClock.Instant.useManualTime(base)
        XCTAssertEqual(NetworkClock.Instant.systemNowAbsolute, base)
    }
    ...
    func testUseManualTimeKeepsContinuousAndAbsoluteSeparate() {
        ...
        XCTAssertEqual(NetworkClock.Instant.systemNow, base)
        XCTAssertEqual(NetworkClock.Instant.systemNowAbsolute, absolute)
    }
    ...
    func testUseManualTimeOverwritesAPreviousManualTime() {
        ...
        XCTAssertEqual(NetworkClock.Instant.systemNow, later)
        XCTAssertEqual(NetworkClock.Instant.systemNowAbsolute, later)
    }
    ...
    func testAdvanceManualTimeMovesBothClocks() {
        ...
        XCTAssertEqual(NetworkClock.Instant.systemNow, base.advanced(by: .milliseconds(250)))
        XCTAssertEqual(NetworkClock.Instant.systemNowAbsolute, absolute.advanced(by: .milliseconds(250)))
    }
    ...
    func testAdvanceManualTimeAccumulates() {
        ...
        XCTAssertEqual(NetworkClock.Instant.systemNow, base.advanced(by: .milliseconds(300)))
    }
    ...
    func testAdvanceManualTimeKeepsNanosecondResolution() {
        ...
        XCTAssertEqual(NetworkClock.Instant.systemNow.time, .nanoseconds(2))
    }

    func testDurationIsMeasuredAcrossManualAdvances() {
        NetworkClock.Instant.useManualTime(base)
        let start = NetworkClock.Instant.systemNow
        ...
        XCTAssertEqual(start.duration(to: NetworkClock.Instant.systemNow), .milliseconds(5))
    }
    ...
    func testUseSystemTimeRestoresTheSystemClock() {
        ...
        let restored = NetworkClock.Instant.systemNow
        XCTAssertNotEqual(restored, base)
        usleep(1)
        let next = NetworkClock.Instant.systemNow
