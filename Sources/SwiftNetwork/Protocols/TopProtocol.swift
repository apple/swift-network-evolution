...
extension TopProtocolHandler where Self: ~Copyable {
    ...
    /// Detaches the lower protocol and hands this instance's event state back to the context.
    ///
    /// Nothing sits above a top protocol, so no upper protocol's detach hands its event state back:
    /// whoever owns the instance calls this when it is done with it. The retirement is deferred to
    /// whichever call still holds the state, so this is safe even when reached from inside this
    /// instance's own call.
    public mutating func teardown(in eventContext: inout NetworkContext.EventContext) {
        guard !identifier.isNone else { return }
        do throws(NetworkError) {
            try invokeDetach(in: &eventContext)
        } catch {
            Logger.proto.error("Failed to detach lower protocol: \(error)")
        }
