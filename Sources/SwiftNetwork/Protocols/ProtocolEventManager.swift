...
struct ProtocolEventManagerState: ~Copyable {
    ...
    // timer wakeup, any such block currently running, any event delivered to this state but
    // not yet drained, and any event queued for this state in another state's
    // `pendingEventsToDeliverToUpperProtocol`. They all dereference the index, so the event state
    // must outlive them; see `retiring`.
...
extension NetworkContext.EventContext {
    ...
    // Takes a hold on the event state an event sitting in a `pendingEventsToDeliverToUpperProtocol`
    // queue will eventually be delivered to, and returns that index so the hold can be released.
    @inline(always)
    fileprivate mutating func holdQueuedEventTarget(
        _ event: borrowing ProtocolEventManagerState.PendingEvent
    ) {
        let index = event.toInstance.protocolEventStateIndex
        guard !index.isNone else { return }
        protocolEventStates[index].addOutstandingHolder()
    }

    // Releases the hold `holdQueuedEventTarget` took, dropping the event state if that was the
    // last thing keeping a retiring one alive.
    @inline(always)
    fileprivate mutating func releaseQueuedEventTarget(_ index: NetworkStateIndex) {
        guard !index.isNone else { return }
            ...
            while upperEvents > 0 {
                if upperEvents == 1 {
                    ...
                    if let pendingEvent = protocolEventStates[index].readPendingEventToUpper() {
                        // Released only once the event has run, so the target outlives a teardown
                        // the event itself triggers.
                        let queuedTarget = pendingEvent.toInstance.protocolEventStateIndex
                        runEvent(pendingEvent)
                        releaseQueuedEventTarget(queuedTarget)
                    }
                } else {
                    ...
                    for _ in 0..<upperEvents {
                        if let pendingEvent = protocolEventStates[index].readPendingEventToUpper() {
                            ...
                            // `deliver` took its own hold on the same state, so the queue's hold
                            // has done its job and the state still lives until `runEvents` below.
                            releaseQueuedEventTarget(queuedTarget)
                        }
                    }
    ...
    fileprivate mutating func deliverEventToUpperProtocol(
        ...
        // The queued event names its target by index but won't dereference it until the queue is
        // drained, so keep the target's event state alive until then.
        holdQueuedEventTarget(event)
...
extension InstanceIdentifier {
    ...
    @inline(always)
    func hasQueuedEventsForUpperProtocol(in eventContext: inout NetworkContext.EventContext) -> Bool {
        guard !eventStateIndex.isNone else { return false }
        return !eventContext.protocolEventStates[eventStateIndex].unassignedPendingEventsToDeliverToUpperProtocol
            .isEmpty
    }
