...
extension OneToOneProtocolHandler where Self: ~Copyable {
    ...
    ///
    /// A protocol whose client sits outside the stack has nothing above it to detach it, so whoever
    /// owns the instance calls this when it is done with it. The retirement is deferred to whichever
    /// call still holds the state, so this is safe even when reached from inside this instance's own
    /// call.
    public mutating func teardown(in eventContext: inout NetworkContext.EventContext) {
        guard !identifier.isNone else { return }
        do throws(NetworkError) {
            try lower.invokeDetach(for: effectiveSelfInstance, in: &eventContext)
        } catch {
