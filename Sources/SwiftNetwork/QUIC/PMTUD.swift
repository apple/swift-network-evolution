...
struct PMTUDState: ~Copyable {
    ...
    mutating func start(
        ...
        if timerID == nil {
            ...
            timerID = connection.timer.insert(
                description: "PMTUD",
    ...
    mutating func packetTooBigReceived(
        on path: QUICPath,
        nextMTU: Int,
        in eventContext: inout NetworkContext.EventContext
    ) {
    ...
    private mutating func searchComplete(
        on path: QUICPath,
        in eventContext: inout NetworkContext.EventContext
    ) {
        canProbe = false
