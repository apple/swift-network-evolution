...
// MARK: - TCP settings (CLI-configured)
...
struct TCPSettings {
    ...
    var enableKeepalive: Bool = false
    var keepaliveIdle: UInt32 = 0
...
private func makeUDPConnection(to remote: Endpoint, localEndpoint: Endpoint) -> NetworkConnection<UDP> {
    NetworkConnection(to: remote, using: makeUDPParams(localEndpoint: localEndpoint))
}

// MARK: - TCP helpers

private func configureTCP(_ tcp: TCP, from settings: TCPSettings) -> TCP {
    var mutableTCP = tcp
    if settings.noDelay {
...
private func makeTCPParams(localEndpoint: Endpoint?, settings: TCPSettings) -> ParametersBuilder<TCP> {
