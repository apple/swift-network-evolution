...
extension ProtocolInstance where Self: AnyObject {
    /// Hands back this instance's event state if it is still registered.
    ///
    /// A harness registers its event state when it is built, and one that nothing ever detached is
    /// still registered when it is destroyed, which trips a precondition. A test that keeps a
    /// harness only for the length of the test calls this before letting go of it.
    public func retireEventStateForTest() {
...
public class UpperHarness<LinkageFamily: DataLinkageFamily>: UpperHarnessProtocol {
    ...
    public required init(
        ...
        self.path = path
        log.logPrefix = "[UpperHarness:\(identifier)]"
        self.identifier = .init(eventManager: &self.eventManager, in: &eventContext)
    }
